...
class DB {
    ...
    //метод api вызываем в строке ввода в браузере .../snake-new/api/?method=имя метода
    //если нужны параметра то добавляем их через &
    //Не забудь стереть комментарии!

    //!!!!!!!!!!!!!!!!!!!!!!!!!!! шаг 1
    //пишем по примеру одну из функций, затем переходим в router.php

    //пример без параметра
    //возвращение длины змеии всех активных игроков
    public function getSnakeSumLengthOfOnlineUsers(){
        $sql = 'SELECT count(*) from snake_body'; //SQl запрос
    ...
    public function getUserById($id) {
        $sql = 'SELECT name, login, score FROM user WHERE id = :id'; //Sql запрос
        $stm = $this->conn->prepare($sql);
        $stm->bindValue(':id', $id, PDO::PARAM_INT); // перечисление параметра
        $stm->execute();
        return $stm->fetchObject('stdClass');
    }
